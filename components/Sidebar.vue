<template>
  <!-- Mobile backdrop -->
  <div v-if="isMobileMenuOpen" class="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden" @click="toggleMobileMenu">
  </div>
  ...
  <div
    ...
    <div class="p-3 lg:p-4 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center justify-between mb-3 lg:mb-4">
        ...
        <div class="flex items-center gap-1">
          <!-- Mobile close button -->
          <UButton icon="i-heroicons-x-mark" color="gray" variant="ghost" size="sm" class="lg:hidden"
      ...
      <UButton :loading="loader" icon="i-heroicons-plus-circle" class="w-full justify-center text-sm lg:text-base"
        ...
        <div class="p-2 lg:p-3 space-y-1 lg:space-y-2">
          <div v-if="threads.length === 0" class="text-center py-6 lg:py-8">
            <UIcon name="i-heroicons-chat-bubble-left-ellipsis"
              class="w-10 h-10 lg:w-12 lg:h-12 text-gray-400 mx-auto mb-2 lg:mb-3" />
          ...
          <div v-for="thread in threads" :key="thread.id" class="group relative">
            <UButton variant="ghost" :class="[
              ...
              <div class="flex items-start gap-2 lg:gap-3 w-full min-w-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs opacity-70 truncate mt-1">{{ getModelName(thread.model) }}</p>                </div>
              </div>
            </UButton>
    ...
    <div class="p-3 lg:p-4 border-t border-gray-200 dark:border-gray-700">
      <div class="flex gap-2">
        <UButton icon="i-heroicons-document-magnifying-glass" size="sm" variant="ghost"
          class="flex-1 text-xs lg:text-sm" @click="navigateTo('/logs')">
          <span class="hidden lg:inline">Usage Logs</span>
          <span class="lg:hidden">Logs</span>
        </UButton>
        <UButton icon="i-heroicons-arrow-left-end-on-rectangle" size="sm" variant="ghost" color="red"
          class="flex-1 text-xs lg:text-sm" @click="logout">
...
const getModelName = (model) => {
  ...
  return map[model] || model || 'Claude 3.5 Sonnet';
};
